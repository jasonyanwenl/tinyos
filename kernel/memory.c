...
void *malloc_page(enum pool_flags pf, uint32_t pg_cnt) {
    ...
    while (cnt-- > 0) {
        ...
        if (page_phyaddr == NULL) {
            // TODO: rollback allocated vaddr and paddr
            return NULL;
        }
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    put_str("\n");
    put_str("   kernel_pool_phy_addr_end:"); put_int(kernel_pool.phy_addr_start + kernel_pool.pool_size);
    ...
    put_str("\n");
    put_str("   user_pool_phy_addr_end:"); put_int(user_pool.phy_addr_start + user_pool.pool_size);
    put_str("\n");
...
void *sys_malloc(uint32_t size) {
    ...
    if (size > 1024) {
        ...
        if (list_empty(&descs[desc_idx].free_list)) {
            // No available free block, allocate new page
            a = malloc_page(PF, 1);
            if (a == NULL) {
                lock_release(&mem_pool->lock);
                return NULL;
...
void pfree(uint32_t pg_phy_addr) {
    ...
    if (pg_phy_addr >= user_pool.phy_addr_start) {
        // user physical mem pool
        mem_pool = &user_pool;
        bit_idx = (pg_phy_addr - user_pool.phy_addr_start) / PG_SIZE;
    } else {
        // kernel physical mem pool
        mem_pool = &kernel_pool;
